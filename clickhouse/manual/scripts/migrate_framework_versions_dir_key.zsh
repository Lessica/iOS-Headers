...
SQL_FILE="$ROOT_DIR/clickhouse/manual/005_migrate_framework_versions_dir_key.sql"
...
if compose_clickhouse_ready; then
  ...
  echo "framework directory key migration completed via docker compose"
  exit 0
fi
...
echo "framework directory key migration completed"
